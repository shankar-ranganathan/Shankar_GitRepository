...
public class TwitterWebSocketRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
      ...
      /*  WebsocketComponent wc = getContext().getComponent("websocket", WebsocketComponent.class);
        ...
        TwitterTimelineComponent tc = getContext().getComponent("twitter-timeline", TwitterTimelineComponent.class);
       ...
       /* fromF("twitter-search://%s?delay=%s", searchTerm, delay).convertBodyTo(String.class)
            .to("log:tweet")
            // and push tweets to all web socket subscribers on camel-tweet
            .to("file:F://Softwares//camel-test-outputfolder/output");*/
        
        from("twitter-timeline://user"
        		+ "?type=polling&user=shankar_rangan&delay=12000")
        .to("log:tweet");
        //.to("bean:blah");
    }
}
